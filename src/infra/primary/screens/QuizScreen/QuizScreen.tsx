...
export const QuizScreen = ({ navigation, route }: QuizScreenProps) => {
  const { sourateNumero = 1, isNew = false } = route?.params || {};
  const { t } = useTranslation();
  const { user } = useAuth();
  const { arabicFontSize } = useFontSize();
    ...
    <ScrollView style={styles.container} contentContainerStyle={styles.content}>
      ...
      <View style={styles.questionCard}>
        ...
        <View style={styles.verseBox}>
          <Text style={[styles.verseText, { fontSize: arabicFontSize }]}>{currentQuestion.texteWithBlank}</Text>
        </View>
